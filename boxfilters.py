...
def filters(imagefile):
	...
	typ="gaussiansize"
	kern="Gaussian"
	logblur1=convolution.filter(img1,kern,3,0.6)
	...
	logblur3 =convolution.filter(img1,kern,7,0.6)
	cv2.imwrite(imagename+"_"+typ+"1.jpg",logblur3)
	logblur4=convolution.filter(img1,"LoG",9,0.6)
	cv2.imwrite(imagename+"_"+typ+"4.jpg",logblur4)
	logblur5 =convolution.filter(img1,kern,11,0.6)
